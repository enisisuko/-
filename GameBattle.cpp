...
#include "UI.h"
#include "game.h"
 // Direct3D_GetBackBufferWidth/Height
...
{
    // —— 1) 屏幕尺寸（平台无关）——
    int sw = 1280, sh = 720;                // 兜底
    Battle_GetBackbufferSize(sw, sh);       // 从平台层取，Win=DX/Switch=NVN/GL
    BS.W = (float)sw;
    BS.H = (float)sh;
    ...
    for (int i = 0; i < (int)BS.units.size(); ++i) {
        ...
        if (u.hitFlashT > 0.0f) {
            ...
            DrawSpriteCenterFlip(u.tex, { u.pos.x, u.pos.y - yoff },
                baseW * scaleX * (1.0f + 0.27f * k),
                baseH * scaleY * (1.0f + 0.27f * k),
                u.facingRight, 0.55f * k * a, 1.0f, 0.6f, 0.6f);
        }
        if (u.healFlashT > 0.0f) {
            float k = clamp01(u.healFlashT / 0.25f);
            DrawSpriteCenterFlip(u.tex, { u.pos.x, u.pos.y - yoff },
                baseW * scaleX * (1.0f + 0.36f * k),
                baseH * scaleY * (1.0f + 0.36f * k),
                u.facingRight, 0.45f * k * a, 0.6f, 1.0f, 0.6f);
        }
